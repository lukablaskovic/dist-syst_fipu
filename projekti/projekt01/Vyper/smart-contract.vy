...
@external
def add_entry(_username: String[100], _github_link: String[200]):
    assert len(_username) != 0
    assert len(_github_link) != 0
    self.entries[self.entry_count] = Entry({id: self.entry_count, username: _username, github_link: _github_link})
    self.entry_count += 1
